...
export default {
   ...
   */
  openNote (title) {
    const destination = this.notePathForTitle(title)
  ...
  isNote (filePath) {
    ...
    const ignored = atom.config.get('atom-notes.ignorePaths') || []
    if (normalPath.split(path.sep).some(part => ignored.includes(part))) return false
