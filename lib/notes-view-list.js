...
function getMarkdownPreviewScheme () {
  // NOTE: markdown-preview-plus doesn't seem to support open by URI yet.
  //       For details see: https://github.com/atom-community/markdown-preview-plus/issues/376
  // if (atom.packages.getActivePackage('markdown-preview-plus')) return 'markdown-preview-plus'
...
export default class NotesViewList {
  ...
  didConfirmSelection (item) {
    this.didHide()
    const markdownPreviewScheme = getMarkdownPreviewScheme()
    const ext = path.extname(item.filePath)
    const isMarkdown = ['.markdown', '.md'].includes(ext)
    if (this.openInPreview && isMarkdown && markdownPreviewScheme) {
      atom.workspace.open(`${markdownPreviewScheme}://${encodeURI(item.filePath)}`)
    } else {
