...
export default class NotesViewList {
  ...
  /** Returns a filter list of given items based on the given query.
    *
    * @param {DocumentItem[]} items - List of objects representing users' notes.
    * @param {string} query - User's search input to use in the filtering process.
    */
  filter (items, query) {
    if (!this.isLoaded()) return []
  ...
  autocomplete (query = undefined) {
    ...
    autocompleteTimeout = setTimeout(() => {
      ...
      if (best.title.toLowerCase().startsWith(q.toLowerCase())) {
  ...
  /** Our override for our SelectList's order() callback. */
