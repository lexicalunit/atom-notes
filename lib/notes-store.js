...
async function setupFileWatcher (directoryPath) {
  ...
  return watchPath(watchedDirectory, {}, events => {
    for (const event of events) {
      if (!NotesFs.isNote(event.path)) continue
      ...
      switch (event.action) {
        ...
        case 'renamed':
          ...
          break
        default:
          console.info('atom-notes: unhandled document event', event)
      }
    }
  })
}
...
class NotesStore extends EventEmitter {
  ...
  addDocument (doc) {
    ...
    try {
      ...
      console.error('atom-notes: add document failure', e)
    }
  }
  ...
  updateDocument (doc) {
    ...
    try {
      ...
      console.error('atom-notes: update document failure', e)
    }
  }
  ...
  removeDocument (doc) {
    ...
    try {
      ...
      console.error('atom-notes: remove document failure', e)
    }
  }
