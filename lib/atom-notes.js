...
export default {
  ...
  */
  handleURI (parsedUri) {
    ...
    if (Object.prototype.hasOwnProperty.call(OPTS, pathname)) {
  ...
  deserializeSearchIndex ({ data }) {
    try {
      ...
      if (Object.prototype.hasOwnProperty.call(store, 'corpusTokens')) return null
...
function makeReady (self, state = null) {
  setTimeout(() => {
    ...
    self.storePromise = new Promise(function (resolve, reject) {
      ...
      const store = new NotesStore(directoryPath, extensions, index)
      store.on('ready', () => {
        self.ready = true
        store.loaded = true
...
function ensureNotesDirectory () {
  const notesDirectory = NotesFs.getNotesDirectory()
  const packagesDirectory = fs.normalize(path.join(process.env.ATOM_HOME, 'packages'))
  const defaultNotesDirectory = path.join(packagesDirectory, 'atom-notes', 'notebook')

  if (notesDirectory.startsWith(packagesDirectory)) {
    const msg = `Notes Directory ${notesDirectory} cannot reside within your atom packages directory.`
...
function autosave (paneItem) {
  ...
  const uri = __guard__(paneItem.getURI, f => f.call(paneItem))
  if (!uri) return
  const modified = __guard__(paneItem.isModified, f => f.call(paneItem))
  if (!modified) return
...
async function autodelete (paneItem) {
  ...
  const filePath = __guard__(paneItem.getURI, f => f.call(paneItem))
  if (!filePath) return false
  if (!NotesFs.isNote(filePath)) return false
  const empty = __guard__(paneItem.isEmpty, f => f.call(paneItem))
  if (!empty) return false
  const noteTitle = filePath.substr(filePath.lastIndexOf('/') + 1)
